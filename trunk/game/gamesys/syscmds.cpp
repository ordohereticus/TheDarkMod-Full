...
static bool init_version = FileVersionList("$Id: syscmds.cpp 1431 2007-10-15 19:46:43Z sparhawk $", init_version);
...
void Cmd_InventoryUse_f( const idCmdArgs &args )
{
	...
	if( args.Argc() == 2)
	{
		...
		if (item != NULL)
		{
			...
			player->inventoryUseItem(IS_PRESSED, item->GetItemEntity(), 0);
		}
		else
		{

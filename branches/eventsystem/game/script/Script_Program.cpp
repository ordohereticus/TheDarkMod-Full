/*****************************************************************************
 ...
 $Revision: 5634 $ (Revision of last commit) 
 $Date: 2012-10-31 01:00:39 -0400 (Wed, 31 Oct 2012) $ (Date of last commit)
...
static bool versioned = RegisterVersionedFile("$Id: Script_Program.cpp 5634 2012-10-31 05:00:39Z greebo $");
...
namespace
{
	...
	idStr GetEventDocMediaWiki(const idEventDef* ev, bool includeSpawnclassInfo = true)
	{
		idStr out;
	...
	void WriteMediaWikiDoc(const Eventmap& events, idFile& out, const idStr& dateStr)
	{
		...
		Writeln(out, "");
		Writeln(out, "Generated by " + version + ", last update: " + dateStr);
		Writeln(out, "");
		...
		for (Eventmap::const_iterator i = events.begin(); i != events.end(); ++i)
		{
			...
			Write(out, GetEventDocMediaWiki(ev));
			 ...
			 i != spawnClassEventMap.end(); ++i)
		{
			...
			{
				Write(out, GetEventDocMediaWiki(*t, false));
			}
		}
